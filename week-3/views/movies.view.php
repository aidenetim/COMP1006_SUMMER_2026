            ...
            <section>
                <?php 
                    foreach($lessonMovieRecords as $singleMovieObject){
                        $validatedTite = htmlspecialchars($singleMovieObject->title ?? "Unknown Title");
                        $validatedRelease = htmlspecialchars($singleMovieObject->release_date ?? "N/A");
                        $extractedPoster = $singleMovieObject->poster_path ?? null;

                        $resolveImgUrl = $extractedPoster
                        ? "https://image.tmbd.org/t/p/w500" . htmlspecialchars($extractedPoster)
                        : "https://via.placeholder.com/100x150.png?text=No+Image";
                    
                ?>
                <div>
                    <img class="movie-img" src="<?php echo $resolveImgUrl; ?>" alt="<?php echo $validatedTite; ?>">
                    <h3><?php echo $validatedTite; ?></h3>
